...
$(function () {
  ...
  if (dt_invoice_table.length) {
    var dt_invoice = dt_invoice_table.DataTable({
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a href="app-message-edit.html" class="dropdown-item">Редактировать</a>' +
              '<div class="dropdown-divider"></div>' +
              '<a href="javascript:;" class="dropdown-item delete-record text-danger">Удалить</a>' +
              '</div>' +
              '</div>' +
              '</div>'
            );
      ...
      buttons: [
    //     {
    //       text: '<i class="ti ti-plus me-md-1"></i><span class="d-md-inline-block d-none">Создать сообщение</span>',
    //       className: 'btn btn-primary',
    //       action: function (e, dt, button, config) {
    //         window.location = 'app-message-add.html';
    //       }
    // }
      ],
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="UserRole" class="form-select"><option value=""> Выбрать статус </option></select>'
            )
